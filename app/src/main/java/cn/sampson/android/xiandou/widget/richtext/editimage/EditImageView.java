package cn.sampson.android.xiandou.widget.richtext.editimage;
...
public class EditImageView extends AppCompatImageView implements IUploadEngine.UploadProgressListener {
    ...
    public void setUploadEngine(IUploadEngine engine) {
    ...
    private void resizeImageView(Uri uri) {
        ...
        if (uri != null && YUtils.isLocalUri(uri)) {
            final int[] imgWH = YUtils.getImageSize(getContext(), uri);//获取图片的宽高
            final float whProp = ((float) imgWH[1]) / imgWH[0];//获取图片的比例
            viewHeight = (int) (viewWidth * whProp);
        } else {
    ...
    public void doUpload() {
        if (uploadEngine != null && mUri != null && YUtils.isLocalUri(mUri) && (uploadStatus == 0 || uploadStatus == IUploadEngine.STATUS_UPLOAD_FAIL)) {
    ...
    public String getUrl() {
        return uploadUrl == null ? mUri.toString() : uploadUrl;
    }
}
