package cn.sampson.android.xiandou.widget.timepicker.utils;
...
public class DateUtils extends android.text.format.DateUtils {
    ...
    /**
     * 根据时间戳得到对应的年，月，日，时，分值
     */
    public static int[] getDateTimeIntegerValue(long timestamp) {
        String formatTime = formatDate(new Date(timestamp), "yyyy-MM-dd-HH-mm");
        String[] times = formatTime.split("-");
        int[] values = new int[times.length];
        for (int i = 0; i < times.length; i++) {
            values[i] = Integer.valueOf(times[i]);
        }
        return values;
    }
