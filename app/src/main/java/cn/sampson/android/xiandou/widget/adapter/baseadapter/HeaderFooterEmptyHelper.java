 ...
 */
public class HeaderFooterEmptyHelper {

    private List<View> mHeaders;
    private List<View> mFooter;
    private List<Integer> mLayoutIds;
    private int emptyPageId;
    private boolean ignoreHeaderFooter;

    public void setEmptyPage(int viewId, boolean ignoreHeaderFooter) {
    ...
    public void addHeaderView(int index, View v) {
        ...
        if (!mHeaders.contains(v) && index <= mHeaders.size())
            mHeaders.add(index, v);
    }
    ...
    public int findViewInHeader(View v) {
        if (mHeaders != null && mHeaders.contains(v)) {
            return mHeaders.indexOf(v);
        }
        return -1;
    }
