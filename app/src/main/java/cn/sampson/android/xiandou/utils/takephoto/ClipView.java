...
public class ClipView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (!isSetMargin) {
			...
			clipTopMargin = (height - clipHeight) / 2-100;
		}
		// 画阴影
		paint.setAlpha(130);
		// top
		...
		// bottom
		canvas.drawRect(0, clipTopMargin + clipHeight+(clipWidth - clipHeight), width, height, paint);
		...
		canvas.drawRect(clipLeftMargin, clipTopMargin, clipLeftMargin
				+ clipWidth, clipTopMargin + clipHeight+(clipWidth - clipHeight), borderPaint);
	...
	public int getClipHeight() {
		return clipHeight - clipBorderWidth+(clipWidth - clipHeight);
	}
