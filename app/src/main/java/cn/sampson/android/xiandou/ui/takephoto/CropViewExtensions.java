...
class CropViewExtensions {
    ...
    static void cameraUsing(Activity activity, Uri output, int requestCode) {
        activity.startActivityForResult(
                createCameraIntent(activity, output),
                requestCode);
    }

    private static Intent createCameraIntent(Activity activity, Uri output) {
        Intent cameraIntent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        if (cameraIntent.resolveActivity(activity.getPackageManager()) != null) {
            cameraIntent.putExtra(MediaStore.Images.Media.MIME_TYPE, "image/jpeg");
            cameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, output);
        }
        return Intent.createChooser(cameraIntent, null);
    }
